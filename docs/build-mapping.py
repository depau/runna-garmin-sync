...
R = sys.argv[1]
runna = json.loads(Path(f"{R}/docs/runna-exercise-catalog.json").read_text())["exercises"]
gar = json.loads(Path(f"{R}/docs/garmin-exercises.json").read_text())
all_ids = Path(f"{R}/docs/runna-exercise-ids.txt").read_text().split()
...
def infer_runna_equip(rid):
    t = rid.upper()
    if t.startswith("BARBELL") or "BARBELL" in t:
        return "BARBELL"
    if t.startswith("DUMBBELL") or t.startswith("DB_") or "_DB_" in t or "DUMBBELL" in t:
        return "DB"
    if t.startswith("KB_") or "KETTLEBELL" in t or t.startswith("WEIGHTED"):
        return "KB"
    if t.startswith("BANDED") or "BANDED" in t or t.endswith("_BAND"):
        return "BAND"
    if "SWISSBALL" in t or "SWISS_BALL" in t:
        return "SWISSBALL"
    if "PULL_UP" in t or "CHIN_UP" in t:
        return "PUB"
...
def intensity_of_garmin(cat, name):
    ws = tokens(name)  # ordered: first equipment word wins, deterministically
    for w in ws:  # explicit equipment word wins
        if w in EQUIP_WORDS:
            return EQUIP_WORDS[w]
    if cat == "BANDED_EXERCISES":
        return "banded"
    if "bodyweight" in ws:
        return "bodyweight"
...
KW_CAT = [
    ...
    ("SIDE_RAISE", "LATERAL_RAISE"),
    ("SHRUG", "SHRUG"),
    ("CURL", "CURL"),
    ("TRICEP", "TRICEPS_EXTENSION"),
    ...
    ("DEADBUG", "CORE"),
    ("RUSSIAN_TWIST", "CORE"),
    ("MOUNTAIN_CLIMB", "PLANK"),
    ("HAMSTRING", "LEG_CURL"),
    ("LEG_CURL", "LEG_CURL"),
    ("CLAM", "HIP_STABILITY"),
    ("FIRE_HYDRANT", "HIP_STABILITY"),
    ("ABDUCTOR", "HIP_STABILITY"),
    ("HEEL", "CALF_RAISE"),
    ("TOE_RAISE", "CALF_RAISE"),
    ("STEP_UP", "LUNGE"),
    ("SWING", "HIP_SWING"),
    ("KB_SWING", "HIP_SWING"),
...
def intensity_bonus(runna_int, gi):
    ...
    if runna_int in unloaded and gi in unloaded:
        return 0.18  # banded~bodyweight, both no logged kg
...
def candidate_categories(rid, m, runna_int):
    ...
    for kw, cat in KW_CAT:
        if kw in t and cat not in cats:
            cats.append(cat)
...
def score(rid, m):
    ...
    for c, ekey, name in pool:
        ...
        if rsq and rsq == gsq:
            sub = 0.6
        elif rsq and rsq == gsq2:
            sub = 0.55
        else:
            sub = 0
        namescore = jac + sub
        if namescore == 0:
            continue  # need some name signal
...
CURATED = {
    ...
    "DOUBLE_LEG_CALF_RAISE_ON_STEP": (
        "CALF_RAISE",
        "STANDING_DUMBBELL_CALF_RAISE",
        "med",
        "double-leg weighted calf raise",
    ),
    ...
    "GLUTE_BRIDGE_HAM_WALKOUT": (
        "LEG_CURL",
        "SLIDING_LEG_CURL",
        "med",
        "glute bridge + heel walkout ≈ sliding leg curl",
    ),
    ...
    "SL_DB_RDL_KNEE_DRIVE": (
        "DEADLIFT",
        "SINGLE_LEG_ROMANIAN_DEADLIFT_WITH_DUMBBELL",
        "high",
        "single-leg DB RDL (+knee drive)",
    ),
}
...
for rid in sorted(set(all_ids) | set(meta) | set(CURATED)):
    ...
    if best is not None and best[0] >= 0.30:
        ...
        if not m and conf == "high":
            conf = "med"  # inferred capped at med
